...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # --- PATCH START ---
        self.requested_ids = [0,46,100,143,188,230,275,322,367,410]
        print(f"[lerobot_dataset] Using specific episode IDs: {self.requested_ids}")

        # 1) Keep only the requested episodes (retain global indices)
        valid_ids = [i for i in self.requested_ids if i in self.meta.episodes]
        ...
        # 4) Skip timestamp sync check (subset)
        timestamps = torch.stack(self.hf_dataset["timestamp"]).numpy()
        episode_indices = torch.stack(self.hf_dataset["episode_index"]).numpy()

        # Convert correctly for check_timestamps_sync
        ep_data_index_np = {
        ...
        if len(self.episodes) < getattr(self.meta, "total_episodes", len(self.episodes)):
            print("[lerobot_dataset] Skipping timestamp sync check for filtered subset.")
        else:
            check_timestamps_sync(
