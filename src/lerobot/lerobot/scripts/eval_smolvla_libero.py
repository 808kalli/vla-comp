...
class Args:
    ...
    video_out_path: str = "rollouts/libero/videos_zeroshot/"
    """Path to save videos."""
    device: str = "cuda"
        ...
        for episode_idx in tqdm(
            ...
            while t < max_steps:
                try:
                    ...
                    observation = {
                        "observation.image": torch.from_numpy(agentview_image / 255.0)  #observation.images.image
                        .permute(2, 0, 1)
                        .to(torch.float32)
                        .to(args.device).unsqueeze(0),
                        "observation.image2": torch.from_numpy(wrist_img / 255.0)       #observation.images.wrist_image
                        .permute(2, 0, 1)
                        .to(torch.float32)
                        .to(args.device).unsqueeze(0),
